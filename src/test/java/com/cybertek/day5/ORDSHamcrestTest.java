...
public class ORDSHamcrestTest extends HRTestBase {
    ...
    public void employeesTest2() {
         //WE WANT TO CHAİN AND ALSO GET RESPONSE OBJECT

            JsonPath jsonPath = given().accept(ContentType.JSON).queryParam("q", "{\"job_id\": \"IT_PROG\"}")
                .get("/employees")
                .then().statusCode(200)
                .body("items.job_id", everyItem(equalTo("IT_PROG")))
                .extract().jsonPath();

             // extract method allows us to get response object after we ue then() method
             //assert that we have only 5 firstName
             assertThat(jsonPath.getList("items.first_name"), hasSize(5));

             //assert firstnames order
            assertThat(jsonPath.getList("items.first_name"),containsInRelativeOrder("Alexander","Bruce", "David"));
